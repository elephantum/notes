...
Альтернативой к выкатке руками может быть отдельный процесс, который применяет миграции независимо от запуска приложения (если это кубер, то отдельный Job, который запускается при деплойменте)
